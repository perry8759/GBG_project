...
<body>
	...
	<div class="container col-md-8">
		<div class="card">
			<div class="card-body">
					<form action="<c:url value='/ACT/ACT_Main_Insert2' />" method="post" id="ACTform" enctype='multipart/form-data'>
					<h2>
						<c:if test="${act != null}">
                       		編輯活動表單
					...
					<div class="form-group">
						<label>活動標題:</label> <input type="text" class="form-control"
							name="ACT_TITLE" id="title_input" value="<c:out value='${act.ACT_TITLE}' />">
                    ...
                    <c:if test="${act == null}">
						球類類型:<br>
						<c:forEach var="sport" items="${SPORT}" varStatus="st">
							<input type="radio" name="dos_sport" value="${sport.DOS_SPORT_ID}"> ${sport.DOS_SPORT_NAME}<br>
						</c:forEach>
					</c:if>
					</div>
					<div class="form-group">
						報名費用:<input type="text" class="form-control" name="ACT_PAY" value="<c:out value='${act.ACT_PAY}' />">
						<br>
					</div>
					<div class="form-group">
						參加隊伍上限:<input type="text" class="form-control" name="ACT_MAX_TEAM" value="<c:out value='${act.ACT_MAX_TEAM}' />"><br>
					</div>
					<div class="form-group">
						總人數上限:<input type="text" class="form-control" name="ACT_MAX_PNUM" value="<c:out value='${act.ACT_MAX_PNUM}' />"><br>
					</div>
					<div class="form-group">
					<c:if test="${act != null}">
						場地: ${act.dos_id.DOS_NAME}<input type="hidden" value="${act.dos_id.DOS_ID}"
							name="DOS_ID"><br><!--未來想改成可修改 -->
					</c:if>
                    <c:if test="${act == null}">
                    	場地: ${DOSID.DOS_NAME}<input type="hidden" value="${DOSID.DOS_ID}"
							name="DOS_ID"><br><!--未來想改成可選 -->
                    </c:if>
					</div>
					<div class="form-group">
						比賽文件:<input type="file" class="form-control" name="ACT_RFORM" value="<c:out value='${act_forms}' />"><br>
					</div>
